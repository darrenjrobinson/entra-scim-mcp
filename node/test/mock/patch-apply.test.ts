...
describe("applyUserPatch", () => {
  it("replaces a simple attribute", () => {
    const updated = applyUserPatch(
      makeUser(),
      patch([{ op: "replace", path: "displayName", value: "New" }]),
    );
  ...
  it("replaces a nested attribute (name.givenName)", () => {
    const updated = applyUserPatch(
      makeUser(),
      patch([{ op: "replace", path: "name.givenName", value: "Zed" }]),
    );
  ...
  it("updates the Entra extension via URN path", () => {
    const updated = applyUserPatch(
      makeUser(),
      patch([{ op: "replace", path: `${SCHEMA_ENTRA_USER}:mailNickname`, value: "nn" }]),
    );
  ...
  it("removes an attribute", () => {
    const updated = applyUserPatch(
      makeUser(),
      patch([{ op: "remove", path: "displayName" }]),
    );
...
describe("applyGroupPatch", () => {
  ...
  it("routes single-member removes through the store", () => {
    ...
    applyGroupPatch(
      group,
      patch([{ op: "remove", path: `members[value eq "${u1.id}"]` }]),
      store,
    );
